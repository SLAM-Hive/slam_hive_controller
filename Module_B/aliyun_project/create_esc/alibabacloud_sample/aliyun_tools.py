...
class Esc_Create:
    ...
    def describe_instance_status(
        ...
        while True:
            request = ecs_20140526_models.DescribeInstanceStatusRequest(
                region_id=region_id,
                instance_id=instance_ids[now_start: min(now_start + batch_size, all_size)],
                # 这里有个疑问：如果不设置分页值，可以返回全部吗
            )
            #print(instance_ids[now_start: min(now_start + batch_size, all_size)])
            responces = client.describe_instance_status(request)
            instance_status_list = responces.body.instance_statuses.instance_status
            #print((instance_status_list[0]))
            for i in range(len(instance_status_list)):
                all_instance_status_list.append(instance_status_list[i])
            now_start += 50
            if now_start >= all_size:
                break

        return all_instance_status_list
    ...
    def describe_status_image(
        ...
        try:
            # 复制代码运行请自行打印 API 的返回值
            resp = client.describe_images_with_options(describe_images_request, runtime)
            #print(resp)
            return resp.body.images.image
        except Exception as error:
            # 如有需要，请打印 error
            #UtilClient.assert_as_string(error.message)
            print(error)
