...
if __name__ == '__main__':
    ...
    # # image create test
    # resp = Esc_Create.create_image(client, MASTER_REGION, 'i-8vb587fzwibfecl72gbz', 'master_node_image2')
    # print(resp)
    # image_id = resp
    # while True:
    ...
    #         if image_id == res[i].image_id:
    #             print(res[i].status,"process:",res[i].progress)
    #             new_status = 1
    #             break
    #     res = Esc_Create.describe_status_image(client, MASTER_REGION, "Available")
    #     for i in range(len(res)):
    #         if image_id == res[i].image_id:
    #             print(res[i].status,"process:",res[i].progress)
    #             new_status = 2
    ...
    # # print(Esc_Create.describe_one_image(client, MASTER_REGION, 'm-8vbc1fwipi8ekfuayoye').status)
    ...
    #print("check info time:",time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
    ...
    instance_temp_dict.update({"image_id": 'm-8vbf8aes547johswohyx'}) # TO CHANGE
    ...
    # 2.2 等待节点运行就绪
    # work_node1_id_list = ["i-8vb587fzwibfecl72gbz"]
    while not Esc_Create.check_instance_status(Esc_Create.describe_instance_status(client, MASTER_REGION, work_node1_id_list)):
        time.sleep(2)
        print("starting")
    print("OK")
    #print("end time:",time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
    ...
    # work_node1_inner_IP = kubework_node1_infos[0].vpc_attributes.vpc_attributes.private_ip_address.ip_address[0]
